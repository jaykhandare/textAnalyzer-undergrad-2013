...
import TextTokenizer,TextLexicalRuleParser,TextContextualRuleParser,TextLexiconFast,TextCommonsense,TextLemmatiser

class TextTagger:

    def __init__(self,trace_p=0,TextLemmatiser_handle=None):

        if not TextLemmatiser_handle:
            TextLemmatiser_handle=TextLemmatiser.TextLemmatiser()
        self.theTextLemmatiser=TextLemmatiser_handle
        self.trace_p=trace_p
        self.theTokenizer=TextTokenizer.TextTokenizer()
        self.theLexicon=TextLexiconFast.TextLexiconFast()
        self.theLRP=TextLexicalRuleParser.TextLexicalRuleParser(self.theLexicon)
        self.theCRP=TextContextualRuleParser.TextContextualRuleParser()
        self.theTextCommonsense=TextCommonsense.TextCommonsense(self.theTextLemmatiser,self)
    ...
    def tag_tokenized(self,text,all_pos_p=0,commonsense_p=1):
        _TextA_p=self.theLexicon.all_pos
        ...
        for cps in chmod_dict:
            ...
            if '/' in cps and cps[cps.index('/'):].upper()==cps[cps.index('/'):]:
                ...
                the_tokenizer_dict=_TextA_p(cps)
            ...
            _TextA.append({'word':cps,'pos':chmods,'all_pos':the_tokenizer_dict})
        b_arr={'word':'S-T-A-R-T','pos':'STAART','all_pos':[]}
        _TextA.insert(0,b_arr.copy())
        _TextA.append(b_arr.copy())

        if self.trace_p:
            print "TRACE: [output after lexicon lookup]:\n  ",self.form_output(_TextA)

        for _hugo_p in range(len(_TextA)):
            hash=_TextA[_hugo_p]
            ...
            if (hash['word'][0]in user1):
                _TextA[_hugo_p]['pos']='NNP'
            else :
                _TextA[_hugo_p]['pos']='NN'
            groups_cleaned(_TextA,_hugo_p)
            _TextA[_hugo_p]['all_pos']=['UNK',_TextA[_hugo_p]['pos']]

        if self.trace_p:
            print "TRACE: [output after lexical rules were applied]:\n  ",self.form_output(_TextA)
        self.theCRP.apply_rules_to_all_words_brill(_TextA)
        cp_cleaned=self.form_output(_TextA,all_pos_p)

        if commonsense_p:
            cp_cleaned=self.theTextCommonsense.cs_verify_tagged(cp_cleaned)
        return cp_cleaned
    ...
    def verify_and_repair(self,tagged):
        ...
        self.theCRP.apply_rules_to_all_words_brill(_TextA)
        return self.form_output(_TextA,all_pos_p)
...
if __name__=="__main__":
    ...
    m=TextTagger(trace_p)
    print '*************************\n'
    print 'TextTagger v1.2'
    print '--send bug reports to jayendrakhandare@gmail.com--'
    print '\n'
