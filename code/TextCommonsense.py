...
__version__="1.0"
import zlib
import TextUtils,TextLemmatiser,TextTagger

class TextCommonsense:
    cssdb_filename='CSSDB.MDF'

    def __init__(self,TextLemmatiser_handle=None,TextTagger_handle=None):
        ...
        if not TextTagger_handle:
            TextTagger_handle=TextTagger.TextTagger()
        self.theTextTagger=TextTagger_handle
        self.tag_tokenized=self.theTextTagger.tag_tokenized
        self.lemmatise_word=self.theTextLemmatiser.lemmatise_word
        ps1={}
        _TextA_cleaned=TextUtils.TextUtils().find_file(self.cssdb_filename)

        if not _TextA_cleaned:
            self.build_cs_selection_db()
            _TextA_cleaned=TextUtils.TextUtils().find_file(self.cssdb_filename)
        hash1=open(_TextA_cleaned,'rb')
        cat_p=self.setitem
