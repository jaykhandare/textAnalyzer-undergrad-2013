__author__="EKK SPJ"
__version__="1.0"
import TextLexiconCustom
import sys
import TextSettings,TextUtils

if TextSettings.TextSettings().JYTHON_P:
    import jarray
    import struct
else :
    import array

class TextLexiconFast:
    java_p=TextSettings.TextSettings().JYTHON_P
    auto_load_lexicon=1
    ...
    def __init__(self):
        self.lexicon_custom=TextLexiconCustom.TextLexiconCustom()

        if TextUtils.TextUtils().find_file(self.fast_lexicon_filename+'_1.MDF')!='':
            ...
            self.lexicon_filename=TextUtils.TextUtils().find_file(self.lexicon_filename)

            if self.lexicon_filename=='':
                print "ERROR: could not find %s" % self.lexicon_filename
                print "in current dir, %TextA% or %PATH%"
